...
p_train = pipelayer.pipeline_train(X_batches, y_batches, 1)
np_train = pipelayer.without_pipeline_train(X_batches, y_batches, 1)
p_test = pipelayer.pipeline_test(X_batches, y_batches)
np_test = pipelayer.without_pipeline_test(X_batches, y_batches)

print(f"train: {np_train / p_train}, test: {np_test / p_test}")
...
pipelayer.push_layer(in_features=1000, out_features=500, dtype=dtype)
pipelayer.push_layer(in_features=500, out_features=10, dtype=dtype)
p_train = pipelayer.pipeline_train(X_batches, y_batches, 1)
np_train = pipelayer.without_pipeline_train(X_batches, y_batches, 1)
p_test = pipelayer.pipeline_test(X_batches, y_batches)
np_test = pipelayer.without_pipeline_test(X_batches, y_batches)
